...
/*
    function getItemsLenght() public view returns (uint256) {
        return items.length;
    }


    function getItemsByIndex(uint _index) public view returns(string memory) {
    string memory output="";
    output = string(abi.encodePacked("[", items[_index].id, ",", items[_index].nome, ",",
        items[_index].descricao, ",", items[_index].valor,",", items[_index].valor,",",
        items[_index].status,",", items[_index].img,",", items[_index].vendedor,",",
        items[_index].comprador, "]"));
    return output;
    }
